...
#calculate the order of raw sentences based on the rarity of the tokens in each dataset      
dict_ind_token_neg_log_rarity = {i:sum([math.log(normalized_token_counts[token]) for token in list_tokenized_seqs[i]]) 
                         for i in range(len(list_tokenized_seqs))}
...
sorted_indecies = sorted(dict_ind_token_rarity, 
                         key=lambda k:abs(dict_ind_token_rarity[k])
                        + dict_ind_token_neg_log_rarity[k])
...
sorted_list_train_dataset_raw = list(dict.fromkeys(sorted_list_train_dataset_raw))[22000:741000]

with open('/mnt/storage/nasimb/babylm_data/babylm_10M/all_base_rarity_neg_log_rarity_22k_end_741k.train', 'w') as f:
