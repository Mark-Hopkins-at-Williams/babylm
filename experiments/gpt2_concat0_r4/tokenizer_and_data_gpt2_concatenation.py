...
def create_multiple_files_dataset_dict(changed_datasets, ex, mixed_dataset= None, bos_tok=None, eos_tok=None):
    ...
    train_corpora = [dataset for dataset in train_corpora if dataset not in changed_datasets]
    train_corpora = [f'../babylm_data/babylm_10M/{corpus}.train' for corpus in train_corpora]
    
    if ex == "rarity":
        for change_dataset in changed_datasets:
            train_corpora.append(f"/mnt/storage/nasimb/babylm_data/10M_rarity/{change_dataset}_rarity.train")
    elif ex == "log-rarity":
        for change_dataset in changed_datasets:
            train_corpora.append(f"/mnt/storage/nasimb/babylm_data/10M_log_rarity/{change_dataset}_log_rarity.train")
    elif ex == "mixed-rarity":
        train_corpora.append(f"/mnt/storage/nasimb/babylm_data/10M_two_mixed_rarity/{mixed_dataset}.train")
    elif ex == "mixed-log-rarity":
        train_corpora.append(f"/mnt/storage/nasimb/babylm_data/10M_two_mixed_log_rarity/{mixed_dataset}.train")
