...
def train_model(dataset, ex):
    ...
    raw_datasets = create_multiple_files_dataset_dict(dataset, ex)
    ...
    if ex == "rarity":
        model_name = f"{dataset}-rarity-seed"
    elif ex == "log-rarity":
        model_name = f"{dataset}-log-rarity-seed"
    else:
        model_name = f"all-base-miss-{dataset}-seed"
...
rarity_corpora = [ 'cbt', 'qed', 'simple_wikipedia', 'switchboard', 'wikipedia']    
order_experiments = ["rarity", "log-rarity", "miss"]      
for ex in order_experiments:
    for dataset in rarity_corpora:
        train_model(dataset, ex)
